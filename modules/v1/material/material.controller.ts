...
export const createMaterial = async (req: Request, res: Response) => {
  try {
    ...
    const values: any = {
      ...
      file: file.path,
      category:
        category && category !== "null" && category !== "" && category !== "0"
          ? parseInt(category)
          : null,
      parent_id:
        parent_id &&
...
export const getMaterials = async (req: Request, res: Response) => {
  try {
    ...
    const materials = await db
      ...
      .leftJoin(category, eq(material.category, category.id))
...
export const getMaterialByCategory = async (req: Request, res: Response) => {
  try {
    ...
    const rows = await db
      ...
      .leftJoin(
        materialcategory,
        eq(material.materialcategory, materialcategory.id),
      )
      .where(
        and(eq(material.category, categoryNumber), eq(material.isDeleted, 0)),
      )
      .orderBy(orderByClause);
    ...
    const materialCategoryMap = new Map<
      number | null,
      {
        materialcategory: { id: number | null; title: string | null } | null;
        materials: any[];
      }
    >();
    ...
    for (const row of rows) {
      ...
      const {
        materialcategoryId: _,
        materialcategoryTitle: __,
        ...materialData
      } = row;
      ...
      if (!materialCategoryMap.has(materialCategoryId)) {
        materialCategoryMap.set(materialCategoryId, {
          materialcategory: materialCategoryId
            ? { id: materialCategoryId, title: materialCategoryTitle }
            : null,
          materials: [],
        });
      }
  ...
  try {
    ...
    const values: any = {
      ...
      materialcategory:
        ...
        req.body.materialcategory !== "null" &&
        req.body.materialcategory !== "" &&
        req.body.materialcategory !== "0"
